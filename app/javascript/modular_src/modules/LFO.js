...
function LFO() {
	...
	this.node.type = window.WAVEFORM_TYPE_ENUM[this._waveType].id;
	this.node.start();
	Module.call(this);

	this.$$type.setTitle(window.WAVEFORM_TYPE_ENUM[this._waveType].caption);
}
inherits(LFO, Module);
...
Object.defineProperty(LFO.prototype, 'waveform', {
	...
	set: function (value) {
		this._waveType = value % window.WAVEFORM_TYPE_ENUM.length;
		this.node.type = window.WAVEFORM_TYPE_ENUM[this._waveType].id;
		this.$$type.setTitle(window.WAVEFORM_TYPE_ENUM[this._waveType].caption);
	}
});
